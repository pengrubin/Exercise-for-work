...
logitmfx(mod1,atmean=F,data = training.data)
...
rocplot$auc
plot(rocplot,legacy.axes=T)
logitmfx(mod2,atmean=F,data = training.data)
...
rocplot$auc
plot(rocplot,legacy.axes=T)
logitmfx(mod3,atmean=F,data = training.data)
...
g<-na.omit(g)
#logit model
reg2 <- glm(marriage ~ age_cat + edu_cat + female + p_evang + p_metroarea + 
              kerry_04 + p65over + pbachelors ,
            family=binomial(link="logit"),data=g)
summary(reg2)

reg2 <- glm(marriage ~ age_cat + edu_cat + female + p_evang  + 
              kerry_04 ,
            family=binomial(link='logit'),data=g)
summary(reg2)
...
reg2 <- glmer(marriage ~ age_cat + edu_cat + female + p_evang  + 
                kerry_04 + p65over + pbachelors  + (1|state),
              family=binomial(link="logit"),nAGQ=0,data=g)
summary(reg2)
